...
kable(Table.01, digits = 3, booktabs=TRUE,  
      col.names = c("gSSMU", 
             "SAM sign", 
              "n",
              "mean", 
             ...
             "median LKB",
             "p2"),
      longtable=TRUE, align="lcccccc", caption = "Table 1. Estimates of the mean and standard deviation (sd) of summer Ln(LKB) by gSSMU and SAM sign, derived from Equation (2) of Watters et al. (2020). Median LKB shows Local Krill Biomass in tonnes. Statistical results from the nonparametric one-way Kruskal-Wallis test are reported as follows: p-1 compares SAM sign within each gSSMU, and p-2 compares gSSMUs within each SAM sign. ", escape=FALSE)
...
#table Andy_March_Summer
d.andy1 = read_csv("./A_Results/Model02_Andy/hr_derived_s.csv",col_types="dddfd")
#table SSMUs
d.SSMUs = read_csv("./A_Results/Model02_SSMU/hr_derived_s.csv",col_types="dddfd")
...
\newpage
```{r Table 2 Posterior and posterior predictive probabilities, warning=FALSE, echo=FALSE}
### Replica of Summary Table 1, Watters et al. ----
keepers = c(37:74)
pt = d.org %>%
...
best.case.names = c("Best case", "−0.5°C < ONI < 0.5°C", "ONI ≥ 0.5°C", "LKB > 1Mt", "0.01 < LHR < 0.10", "LHR ≥ 0.10", "Worst case")
...
knitr::kable(table.original.scenario, booktabs=TRUE,  col.names = c("Effects", 
             "Best Case", 
              ...
              "ONI > +0.5°C", 
             "Long-term mean", 
              "Predicted long-term mean"),
             longtable=TRUE, align="lccccc", caption = "Table 2. Original table in Watters et al. 2020.  In this and all subsequent tables below, posterior (Long-term mean) and posterior predictive probabilities (Predicted long-term mean) that expected performance given the effects indicated in the left-most column are less than the expected performance given effects or conditions indicated in the column headings. ONI is the Oceanic Niño Index, LKB is local krill biomass, and LHR is local harvest rate. Worst Case is represented by neutral ONI; LHR  $\\geqslant$ 0.1; and LKB $\\geqslant$ 1Mt.  The Best Case is represented by La Niña conditions, low LKB and low LHR.", escape=FALSE)
...
```{r Table 3 Posterior and posterior predictive probabilities, warning=FALSE, echo=FALSE}
### Replica of Summary Table 1, Watters et al. ----
keepers = c(37:74)
pt = d.SSMUs %>%
  dplyr::group_by(Parameter) %>%
...
knitr::kable(table.original.scenario, booktabs=TRUE,  col.names = c("Effects", 
             ...
             "Long-term mean", 
              "Predicted long-term mean"),
             longtable=TRUE, align="lccccc", caption = "Table 3. Posterior (long-term mean) and posterior predictive (predicted long-term mean) probabilities for spatial model by SSMUs.", escape=FALSE)
...
```{r Overwinter penguin  plots, fig.align="centre", out.width="60%", echo=FALSE}
...
```{r Watters original and SSMU ALL data, fig.align="centre", fig.asp=1.5, out.width="65%", echo=FALSE, warning=FALSE}

knitr::include_graphics("./A_Results/Watters_model/Imputed/Watters_Penguin_performance_imputed.png")

#knitr::include_graphics("./A_Results/Model02_Andy/Penguin_performance_ALL_mod02_Andy.png")

knitr::include_graphics("./A_Results/Model02_SSMU/Penguin_performance_ALL_mod02_SSMU.png")

knitr::include_graphics("./A_Results/Model02_SSMU_MarchWinter/Penguin_performance_ALL_mod02_SSMU_W.png")


```

Figure 6. Expected performance of penguins, all combinations. A) From Watters et al. 2020 model. B) Summer and Winter distributions modified according to penguins' use of SSMUs (see text). C) Same as B) but March catches allocated to winter.   



```{r Watters original and SSMU Expected performance, fig.align="centre", fig.asp=1.5, out.width="65%", echo=FALSE, warning=FALSE}

knitr::include_graphics("./A_Results/Watters_model/Imputed/Watters_Penguin_performance.png")

#knitr::include_graphics("./A_Results/Model02_Andy/Penguin_performance_mod02_Andy.png")

#knitr::include_graphics("./A_Results/Model02_Andy/Penguin_performance_mod02_Andy_MWinter.png")

knitr::include_graphics("./A_Results/Model02_SSMU/Penguin_performance_mod02_SSMU.png")

knitr::include_graphics("./A_Results/Model02_SSMU_MarchWinter/Penguin_performance_mod02_SSMU_W.png")


```


Figure 7. Expected performance of penguins attributable to the marginal effects of the Oceanic Niño Index (ONI), local krill biomass (LKB) and local harvest rate (LHR) relative to the “best case”. The best case is ONI ≤ −0.5 °C; LKB ≤ 1 Mt; and LHR ≤ 0.01. The “worst case” is −0.5 °C < ONI < 0.5 °C; LKB > 1 Mt; and LHR ≥ 0.1. The median, interquartile range, and range of the posterior expectations are indicated by each boxplot. Reference lines respectively indicate the median expected performance in the best case (dashed line) and the long-term mean performance. A) Watters et al. 2020 model. B) Summer and Winter distributions modified according to penguin's use of SSMUs (see text). C) Same as B) but March catches allocated to winter. 
...
Figure x. Original Figure 1 plot from Watters et al. 2020 with A)
...
Figure x2. Model output for the alternatve Watters et al. 2020 scenario
...
Figure x3. Model output for Watters et al. 2020 Scenario #2 outlined
