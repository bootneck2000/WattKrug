...
```{r winter Peng indices1 vs LKB LHR, fig.show="hold", out.width="90%", echo=FALSE}
...
```{r winter Peng indices2 vs LKB LHR, fig.show="hold", out.width="90%", echo=FALSE}
...
associated with intermediate ONI conditions, with intermediate (neutral) ONI linked to poorer performance across cases. Secondarily, intermediate LHR yielded lower performance within each ONI category. High
...
close to the long-term mean (Figures S5 and S7) and unsupported by
...
\newpage
# E. Continuing spatial and temporal mismatches
...
knitr::kable(ssmu_table, digits = 3, booktabs=TRUE,  
      col.names = c("SSMU", 
             "Area (km2)", 
              "gSSMU",
              "Area fraction"),
      longtable=TRUE, align="lccc", caption = "Table S5. Size of SSMUs for Subarea 48.1 and its relative size to the gSSMU where they are located. CCAMLR Small-Scale Management Units (SSMUs) were download from CCAMLR Public repository (https://github.com/ccamlr/data).
...
knitr::include_graphics("./A_Results/Model02_SSMU_MarchWinter/Posterior_predictive_dist_mod02_SSMU_W.png")

```

Figure S13. Posterior predictive distributions (boxplots) and jittered observations (colored circles) of penguin  performance. A) Watters model. B) Model SSMU. C) Model SSMU + 03W.


\newpage
# F. Krill consumption by main predator taxa

Krill consumption by major predator groups during the study period was summarized by Hill et al. (2007). During summer, fish accounted for the largest proportion of krill consumption, with estimated removals of 
`r scales::comma(round(sum(Table.06$Fish_Krill_ton[3:4]), 0))` in the
Bransfield Strait (gSMSMU #1: APBSW + APBSE) and
`r scales::comma(round(sum(Table.06$Fish_Krill_ton[1:2]), 0))` in the
...
in the Drake Passage (Table S6). Total summer krill consumption therefore exceeded 1 million t (Mt) in both gSSMU #1 and gSSMU #2.

Watters et al. (2020) concluded that krill biomass at the Antarctic Peninsula is generally sufficient to support penguin production. Consistent with this, no evidence of acute adult penguin mortality or complete absence of birds at nesting grounds has been documented in the region, in contrast to patterns reported for other penguin species during severe starvation events (Vianna et al. 2014; Crawford et al. 2025). Summer survey estimates below 1 Mt for each gSSMU are therefore inconsistent with observed predator dynamics and likely underestimate krill availability.

Winter consumption estimates were lower and more uncertain. Hill et al.
(2007) estimated fish consumption at
...
thus exceeded 500,000 t and 350,000 for gSSMU #1 and #2.
respectively (Table S7).

Since the Hill et al. (2007) estimates, survey-based population assessments indicate that humpback whale abundance has increased approximately 13-fold since 1990, and fin whale abundance approximately 9-fold since 2000 (Table S8). These increases suggest that whale krill consumption at the Antarctic Peninsula is substantially higher today than reflected in the Hill et al. (2007) estimates.
...
Predators_summer <- Predators_summer %>% dplyr::select(-Fish_Qsummer_g, -Whale_Qsummer_g, -Penguin_Qsummer_g, -Reference)

flextable(Predators_summer) |>
  fontsize(size = 8, part = "all") |>
  padding(padding = 1, part = "all") |>
  autofit() |>
  set_caption("Table S6. Estimated krill consumption by major predator taxa during the
...
Predators_winter <- Predators_winter %>% dplyr::select(-Fish_Qwinter_g, -Whale_Qwinter_g, -Penguin_Qwinter_g, -Reference)

flextable(Predators_winter) |>
  fontsize(size = 8, part = "all") |>
  padding(padding = 1, part = "all") |>
  autofit() |>
...
```{r Whale abundance, results='asis', warning=FALSE, echo=FALSE, message=FALSE, comment=NA}

Whales <- read.csv("./A_Results_Suppl/Baleen Whale Abundance.csv",
...
flextable(Whales) |>
  ...
  padding(padding = 1, part = "all") |>
  autofit() |>
  set_caption("Table S8. Estimated whale abundance for gSSMU #1 and #2 during the study period.")
