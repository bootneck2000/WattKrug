...
kable(shapiro_results1, digits = 3, booktabs=TRUE,  
      col.names = c("gSSMU", 
             "SAM sign", 
              ...
              "W", 
             "p"),
      longtable=TRUE, align="lcccc", caption = "Table S1. Shapiro–Wilk test results for normality of log-transformed observed LKB values (lnLKB), stratified by gSSMU and SAM sign. Columns report sample size (n), the Shapiro-Wilk W statistic, and the associated p-value.", escape=FALSE)
...
Figure S2. Frequency distribution of summer krill biomass (LKB; left panels) and summer harvest rate (LHR; right panels). Rows show imputed values from Watters et al. (2020) (top; blue), observed survey data filtered by survey coverage (middle; red), and Model SSMU estimates (this study; bottom; green). Results are shown separately for each SAM sign and gSSMU combination. 
...
Figure S3. Frequency distribution of LKB categories (left panels) and LHR categories (right panels). Rows show imputed values from Watters et al. (2020) (top; blue), observed survey data filtered by survey coverage (middle; red), and Model SSMU estimates (this study; bottom; green). Results are shown separately for each SAM sign and gSSMU combination.
